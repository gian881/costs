...
export function ProjectPage() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useQuery(["project", id], () => getProjectById(id as string));
  ...
  function removeService(id: string, cost: number) {
    ...
    projectUpdated.services = projectUpdated.services.filter(
          ...
          {showProjectForm ? (
            ...
            <div className="w-full mt-4">
              <p className="mb-2">
                ...
                {(project as Project).category.name}
              </p>
              <p className="mb-2">
                <span className="font-bold">Total budget:</span> R${" "}
                {(project as Project).budget.toFixed(2).replace(".", ",")}
              </p>
              <p className="mb-2">
                <span className="font-bold">Budget utilized:</span> R${" "}
                {(project as Project).cost.toFixed(2).replace(".", ",")}
              </p>
            </div>
          )}
        </div>
